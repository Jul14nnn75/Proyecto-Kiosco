...
class InventarioApp:
    def __init__(self, root):
        ...
        # Stock
        tk.Label(frame_datos, text="Stock:").grid(row=4, column=0, sticky="w")
        self.stock_entry = tk.Entry(frame_datos, width=30)
        self.stock_entry.grid(row=4, column=1, padx=5, pady=2)
         
        tk.Label(frame_datos, text="Proovedor:").grid(row=5, column=0, sticky="w")
        self.proovedor_entry = tk.Entry(frame_datos, width=30)
        self.proovedor_entry.grid(row=5, column=1, padx=5, pady=2)


        # ---------- Botones ----------
        frame_botones = tk.LabelFrame(root,text="Botonera", padx=10, pady=10)
    ...
    def agregar_producto(self):
        ...
        codigo_articulo = self.precio_entry.get()
        stock = self.stock_entry.get()
        proveedor = self.proovedor_entry.get()
        ...
        try:
            ...
            query = "INSERT INTO producto (nombre, id_categoria, codigo_articulo, precio, proveedor) VALUES (%s, %s, %s, %s, %s, %s)" #saque stock por que es una tabla aparte 
            valores = (nombre, id_categoria, codigo_articulo, float(precio), proveedor)
