...
if TYPE_CHECKING:
    from caja import CajaApp #Le agregue esto si no el CajaApp queda subrayado en amarillo
...
class POSWindow(tk.Toplevel):
    def __init__(self,caja: "CajaApp" = None): #-------ESTO LO MODIFIQUE---------------
        super().__init__(caja) #-------TMB MODIFIQUE-----------
        self.caja = caja #-------ESTO AGREGUE-------------
    ...
    def registrar_pago(self, tipo):
        ...
        if self.caja and self.caja.monto_apertura is not None:
            self.caja.monto_apertura += monto
